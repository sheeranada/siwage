...
@section('content')
    ...
    <div class="form-tambah d-flex flex-col justify-content-end align-items-center mb-3 gap-4 h-100">
        ...
        <x-modal id="exampleModal" title="Tambah Data Status Keluarga" form-action="{{ route('status_keluarga.store') }}"
                            ...
                            <div class="opsi d-flex justify-content-center align-items-center gap-2">
                                <div class="update">
                                    <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#status_keluarga-{{ $item->id }}">
                                        <i class="fas fa-pen-to-square"></i>
                                        Edit
                                    </button>
                                    <x-modal id="status_keluarga-{{ $item->id }}" title="Tambah Data Talenta"
                                        form-action="{{ route('status_keluarga.update', $item->id) }}" method="PUT"
                                        submit-text="Update">
                                        <div class="mb-3">
                                            <label for="status_keluarga" class="form-label">Status Keluarga</label>
                                            <input type="text" name="status_keluarga" class="form-control"
                                                value="{{ $item->status_keluarga }}" required>
                                        </div>
                                    </x-modal>
                                </div>
                                {{-- delete --}}
                                <div class="delete">
                                    <form action="{{ route('status_keluarga.destroy', $item->id) }}" method="POST"
