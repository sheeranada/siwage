...
@section('content')
    <x-card>
        <x-slot name="header">
            <x-modal id="tambahPendidikan" btn-label="Tambah Data" action="{{ route('pendidikan.store') }}" method="POST"
                icon="bi-plus" btn="success">
                <x-form.input name="id" label="Id" value="{{ $pendidikan->id ?? '' }}" type="text" />
                <x-form.input name="pendidikan" label="Pendidikan" value="{{ $pendidikan->pendidikan ?? '' }}"
                    type="text" />
            </x-modal>
        </x-slot>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center align-items-center gap-2">
                                    <div class="edit">
                                        <x-modal id="editPendidikan-{{ $item->id }}" btn-label=""
                                            action="{{ route('pendidikan.update', $item->id) }}" method="PUT"
                                            icon="fa-pen" btn="warning">
                                            <x-form.input name="id" label="Id" value="{{ $item->id ?? '' }}"
                                                type="text" />
                                            <x-form.input name="pendidikan" label="Pendidikan"
                                                value="{{ $item->pendidikan ?? '' }}" type="text" />
                                        </x-modal>
                                    </div>
